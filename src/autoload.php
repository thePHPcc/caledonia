<?php
// @codingStandardsIgnoreFile
// @codeCoverageIgnoreStart
// this is an autogenerated file - do not edit
require __DIR__ . '/exceptions/Exception.php';
require __DIR__ . '/exceptions/AmountMustNotBecomeNegativeException.php';
require __DIR__ . '/exceptions/AmountMustNotBeNegativeException.php';
require __DIR__ . '/goods/Good.php';
require __DIR__ . '/goods/Bread.php';
require __DIR__ . '/goods/Cheese.php';
require __DIR__ . '/goods/Grain.php';
require __DIR__ . '/market/Market.php';
require __DIR__ . '/goods/Milk.php';
require __DIR__ . '/player/Money.php';
require __DIR__ . '/exceptions/NameMustNotBeEmptyException.php';
require __DIR__ . '/player/Player.php';
require __DIR__ . '/market/Price.php';
require __DIR__ . '/exceptions/PriceMustBePositiveException.php';
require __DIR__ . '/market/PriceTable.php';
require __DIR__ . '/goods/Whisky.php';
require __DIR__ . '/goods/Wool.php';
// @codeCoverageIgnoreEnd
